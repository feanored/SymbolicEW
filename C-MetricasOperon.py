...
def aggregate_and_analyze_metrics(results_subfolder, col_x=''):
    ...
    csv_files = [f for f in os.listdir(results_subfolder) if f.startswith(f'metrics_{col_x}') and f.endswith('.csv')]
    if not csv_files:
    ...
    expected_columns = ['Model', 'popsize', 'complexity', 'R2 Train', 'MSE Train', 'R2 Test', 'MSE Test']
    ...
    grouped = combined_df.groupby(['popsize']).agg({
        'R2 Train': 'mean',
        'R2 Test': 'mean',
        'MSE Train': 'mean',
        'MSE Test': 'mean',
    ...
    print(f"Melhor popsize baseado em MSE Test (menor): {best_mse_test['popsize']} com MSE Test = {best_mse_test['MSE Test']:.4f}")
    ...
    if best_r2_test['popsize'] == best_mse_test['popsize']:
        ...
        grouped['score'] = (grouped['R2 Test'] - grouped['R2 Test'].min()) / (grouped['R2 Test'].max() - grouped['R2 Test'].min()) - \
            (grouped['MSE Test'] - grouped['MSE Test'].min()) / (grouped['MSE Test'].max() - grouped['MSE Test'].min())
        best_combined = grouped.loc[grouped['score'].idxmax()]
        print(f"Popsize recomendado (critério combinado): {best_combined['popsize']} com score = {best_combined['score']:.4f}")
...
def load_metrics_for_col(results_subfolder, col_x=''):
    ...
    if list(df.columns) != expected_columns:
        raise ValueError(f"Header inconsistente em {results_subfolder}: {list(df.columns)}")
    return df
...
def compare_folders(folder_a, folder_b, col_x='', save_prefix='comp_bic_r2'):
    ...
    agg_a = df_a.groupby('popsize').agg({
        'complexity': 'median',
        'R2 Test': 'median',
        'MSE Test': 'median'
    ...
    agg_b = df_b.groupby('popsize').agg({
        'complexity': 'median',
        'R2 Test': 'median',
        'MSE Test': 'median'
    }).reset_index().rename(columns={
...
def compare_popsizes_by_model(folder_a, folder_b, col_x='', save_prefix='comp_popsizes'):
    ...
    for col_idx, (folder, df, label) in enumerate([
        ...
        model_order = (df[df['popsize'] == first_pop]
                       .sort_values('Model')['Model'].tolist())
        if not model_order:
        ...
        for row_idx, (metric_col, metric_label) in enumerate(metrics):
            ...
            for pop_idx, popsize in enumerate(popsizes):
                ...
                ax.plot(x, df_pop[metric_col].values,
                        marker=marker, linestyle=ls, label=f'pop={popsize}')
...
def compare_model_by_popsizes(folder_a, folder_b, col_x='', model=None, save_prefix='comp_model_popsizes'):
    ...
    if model is None:
        print("Informe o parâmetro 'model'. Modelos disponíveis:", sorted(df_a['Model'].unique()))
        return
    ...
    for row_idx, (metric_col, metric_label) in enumerate(metrics):
        ...
        for df, label, color, marker in [
            ...
            df_grouped = (df.groupby('popsize', as_index=False)
                            .mean(numeric_only=True)
                            .sort_values('popsize')
                            .set_index('popsize')
                            .reindex(popsizes))
            ax.plot(list(x_pos), df_grouped[metric_col].values,
...
if __name__ == "__main__":
    ...
    compare_model_by_popsizes("results/by_bic", "results/by_r2", "atflux", model="oiii_5007_ew_mean")
    compare_model_by_popsizes("results/by_bic", "results/by_r2", "aZmass", model="oiii_5007_ew_mean")
    compare_model_by_popsizes("results/by_bic", "results/by_r2", "mass", model="oiii_5007_ew_mean")