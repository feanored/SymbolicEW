...
class PlotsMetricas(object):
    ...
    def plot_corr(
        self, correlacoes, ax, lbls=None, title=None, type="tot", vmin=0, vmax=1
    ):
    ...
    def binned_statistic(self, x, y, nbins=125, tipo="freq"):
        ...
        corr_value, _ = stats.spearmanr(
            bin_centers, medias
        )  # segundo resultado é o p-value
    ...
    def plot_medias_bins(self, dados, col_x, col_y, nbins=125, tipo="freq"):
        centers, medias, stds, corr = self.binned_statistic(
            dados[col_x], dados[col_y], nbins, tipo
        )
    ...
    def juntar_amostras(self, col_x):
        ...
        df_diagramas = df_nii.drop(columns=[T.nii.value]).merge(
            df_oiii[["index"]], on=["index"]
        )
        df_diagramas = df_diagramas.merge(df_nii[["index", T.nii.value]], on=["index"])
        df_diagramas = df_diagramas.merge(
            df_oiii[["index", T.oiii.value]], on=["index"]
        )
        ...
        df_diagramas[T.nii_ha.value] = (
            df_diagramas[T.nii.value] - df_diagramas[T.ha.value]
        )
        df_diagramas[T.oiii_hb.value] = (
            df_diagramas[T.oiii.value] - df_diagramas[T.hb.value]
        )
        return df_diagramas
    ...
    def bins_and_medians(self, x, y, M):
        ...
        return (
            pd.DataFrame({"center": bin_centers, "median": medianas, "mads": mads})
            .dropna()
    ...
    def process_metricas(self, metricas):
        ...
        bests = metricas.loc[metricas.groupby("complexity")["loss"].idxmin()]
    ...
    def get_operon_func(self, target, feature, complexity):
        ...
        if complexity == -1:
            complexity = np.max(df_operon["complexity"])
        scores_operon = df_operon[df_operon["complexity"] == complexity].iloc[0]
        operon_expr = sp.latex(
            sp.N(sp.sympify(scores_operon["equations"].replace("X1", "x")), 4)
        )
        operon_func = sp.lambdify(
            sp.symbols("x"),
            sp.sympify(scores_operon["equations"].replace("X1", "x")),
            "numpy",
        )
    ...
    def get_latex_table(self, df, col_x, col_y):
        ...
        for _, row in df.iterrows():
            linha = [
                f"${sp.latex(sp.N(sp.sympify(row['equations'].replace('X1', 'x')), 3))}$"
            ]
    ...
    def filtra_median(self, df_median, filtro):
        df_median = df_median[
            (df_median["center"] >= filtro[0]) & (df_median["center"] <= filtro[1])
        ]
    ...
    def calcula_corte_outliers(self, medians, limiar=1):
        ...
        corte = np.max(
            medians[
                (np.abs(3 * medians["mads"] / medians["median"]) > limiar)
                & (medians["center"] < meio)
                & (medians["mads"] > 0)
            ]["center"]
        )
    ...
    def calcula_corte_bayes(self, df, alpha0=1e-3, beta0=1e-3, prior="uniform"):
        ...
        def log_marg(k):
            ...
            if alpha0 == 0 and beta0 == 0:
                ...
                lg1 = gammaln(alpha0 + n1 / 2.0) - (alpha0 + n1 / 2.0) * np.log(
                    beta0 + S1 / 2.0
                )
                lg2 = gammaln(alpha0 + n2 / 2.0) - (alpha0 + n2 / 2.0) * np.log(
                    beta0 + S2 / 2.0
                )
            return lg1 + lg2
        ...
        posterior_df = pd.DataFrame(
            {"x0": x0_grid, "log_post": log_post, "posterior": post}
        )
    ...
    def plot_tstudent_fit(self, medians):
        ...
        print(
            f"Parâmetros estimados: graus de liberdade={df:.3f}, loc={loc:.3f}, scale={scale:.3f}"
        )
    ...
    def plotly_entropy(self, df, loss="loss"):
        ...
        for i in range(1, len(df)):
            ...
            if x2 - x1 != 0:  # Evitar divisão por zero
                inclinacao = (
                    -np.arctan((y2 - y1) / (x2 - x1) / max(df[loss])) * 180 / np.pi
                )
            else:
                inclinacao = 0
        ...
        for i in range(1, len(inclinacoes)):
            if (
                inclinacoes[i] < np.quantile(inclinacoes, 0.50)
                or inclinacoes[i] < 0
                or inclinacoes[i - 1] < -1
            ):
                continue
            fig.add_annotation(
                x=x_mids[i],
        ...
        fig.update_xaxes(tickvals=df["complexity"])
        fig.update_layout(showlegend=False, xaxis=dict(showgrid=True))
    ...
    def plot_median_fn(
        self, dados, x, y, complexity, quantil=None, corte=None, titulo="", save=False
    ):
        median = self.bins_and_medians(dados[x], dados[y], 1000)
        ...
        fig, ax = plt.subplots(
            2, 1, gridspec_kw={"height_ratios": [4, 1]}, figsize=(8.5, 5.5), sharex=True
        )
        ...
        ax[0].scatter(
            x=median["center"][ajuste], y=median["median"][ajuste], s=2, color="red"
        )
        ax[0].scatter(
            x=median["center"][~ajuste],
            y=median["median"][~ajuste],
    ...
    def plot_median_mad_movel(self, medians, col_x, col_y, corte=None):
        ...
        plt.scatter(
            x=medians["center"], y=medians["median"] - medians["mads"], s=2, color="red"
        )
        ...
        plt.vlines(
            ...
            color="purple",
            label="%s ~ %.2f \nBP p-value: %.4f" % (col_x, corte, pvalue_antes),
        )
        plt.xlabel(col_x)
        plt.ylabel(col_y)
    ...
    def plot_col_cortes(self, medians, col_x, col_y, cortes=None):
        ...
        if cortes is not None:
            ...
            plt.vlines(
                ...
                np.max(medians["median"]),
                color="blue",
                label="CI-95%%: [%.2f,%.2f]" % (cortes[1], cortes[2]),
            )
            plt.vlines(
                cortes[2],
                np.min(medians["median"]),
    ...
    def plot_bar(self, forests, knn, operon, linhas, metrica, vmax):
        ...
        if operon is not None:
            ax.barh(x - width, operon, width, label="Operon")
    ...
    def plot_KeKa(self):
        ...
        plt.plot(
            x_ke01, ke_01, "--", color="black", alpha=0.5, label="Ke01"
        )  #: extreme starburst >")
        plt.plot(
    ...
    def plot_bpt(
        ...
        if save:
            plt.savefig(
                "%sbpt_%s.png" % ("./bpts/", label.replace("(", "_").replace(")", "_")),
                dpi=100,
            )
        else:
            plt.show()
    ...
    def bpt_mediana_ind(self, dados, col_x, complexity, filtro=(-100, 100), save=False):
        ...
        scores_nii, _, operon_f_nii = self.get_operon_func(
            T.nii.value, col_x, complexity[0]
        )
        scores_ha, _, operon_f_ha = self.get_operon_func(
            T.ha.value, col_x, complexity[1]
        )
        scores_oiii, _, operon_f_oiii = self.get_operon_func(
            T.oiii.value, col_x, complexity[2]
        )
        scores_hb, _, operon_f_hb = self.get_operon_func(
            T.hb.value, col_x, complexity[3]
        )
